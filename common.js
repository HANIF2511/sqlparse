...
//console.log ("-----****---:actionsArr:",actionsArr," queryData :", queryData," rule:", rule , " action:" ,action, " Trigger:",Trigger, "  hostsArr:",hostsArr," hostsMapArr:",hostsMapArr);
			...
			function  handleResponseEnd(ruleLog, msgResponse){
				...
				LogRule(ruleLog, msgResponse, status);
		...
		function LogRule(ruleLog, msgResponse, status)
		{
...
//			logDataStr = logDataStr.split("'").join('"');
			...
			statement = statement + " ( RULE_KEY ,STATUS, MODULE ,RULE_ID ,ACTION_ID ,SENT_DATE ,MSG_RECEIVED ,PARAMETER_SENT, BODY_SENT , MSG_RESPONSE ,LOGDATE ,LOGNAME )";
			statement = statement + " values ( ?, ? , ?, ? , ? , ? , ? , ?, ? , ? , ?, ? )";
			logging.log (logging.getThisLine(),'info', 'statement:' + statement);

			db.query(statement, [ruleKey ,status,  ruleLog.rule.MODULE, ruleLog.rule.RULE_ID, ruleLog.action.ACTION_ID, ruleLog.sentDate, 
						...
						var reqNew = http.request(optionsGlob, (response) => {
						  ...
						  response.on('end', function () {
							...
							response.on('error', (error) => {
								...
								LogRule(ruleLogGlob, msg + " Error:" + err, 400 );
								reject(error);
							});
						});
			...
			if (valid)
			{
				...
				else
				{
					...
					reqNew.on('error', function(err) {
						...
						msg = "Error sending to Host :" +sendTo ;
						console.log( msg + " Error:" + err );
						LogRule(ruleLog, msg + " Error:" + err, 400 );
					});
